...
public class ListaDeEnterosEnlazada extends ListaDeEnteros {
	...
	public ListaDeEnterosEnlazada(int unNumero) {
		this.agregarFinal(unNumero);
		this.comenzar();
	}
	...
	// Custom para el Ejercicio 1.5 - Devuelve una copia de la lista con el siguiente al nodo actual
	public ListaDeEnterosEnlazada siguiente() {
		ListaDeEnterosEnlazada unaLista = new ListaDeEnterosEnlazada();
		unaLista.actual = this.actual.getSiguiente();
		return unaLista;
	}
	
	// Ejercicio 1.5
	public void imprimirInversa(ListaDeEnterosEnlazada unaLista) {
		if (!unaLista.fin()) {
			this.imprimirInversa(unaLista.siguiente());
			System.out.print(unaLista.actual.getDato() + " | ");				
		}
	}
}
