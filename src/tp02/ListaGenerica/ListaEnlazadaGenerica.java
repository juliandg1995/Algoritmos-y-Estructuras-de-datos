...
public class ListaEnlazadaGenerica<T> extends ListaGenerica<T> {
	...
	public boolean agregar( T[] unArreglo ) {
		
		int tamanioInicial = this.tamanio();
		boolean OK = false;
		
		for (int i = 0; i < unArreglo.length; i++) {
			this.agregarFinal(unArreglo[i]);
		}
		
		if(tamanioInicial + unArreglo.length == this.tamanio()) {
			OK = true;
		}
